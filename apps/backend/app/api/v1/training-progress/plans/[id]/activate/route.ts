...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const plan = await TrainingProgressService.activatePlan(id);
    return jsonResponse(plan);
  } catch (e: any) {
    if (e.message === "Plan not found") return errorResponse("Plan not found", "NOT_FOUND", 404);
    if (e.message?.includes("locked")) return errorResponse("Only locked plans can be activated", "INVALID_STATE", 422);
